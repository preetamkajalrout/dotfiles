...
install_vundle() {
  ...
  if [ ! -d "$vundle_dir" ]; then #vundle not found
...
install_nvm() {
  ...
  if [ -d "$dir_nvm_versions" ] && [ ! -z "$(ls -A $dir_nvm_versions)" ]; then # '-z' => string is null, that is, has zero length
    # nvm found
...
install_ycm() {
  ...
  if [ -z "$(dpkg -l | grep vim-youcompleteme)" ]; then
...
install_powerline() {
  ...
  if [ -z "$(pip3 show powerline-status)" ]; then # '-z' => string is null, that is, has zero length
...
install_go() {
  ...
  if [ -d "$dir_go" ]; then                                                # go is installed
    if [ "$(go version | grep -oP 'go[0-9.]+')" != "$latest_gover" ]; then # check if latest go is installed
      is_install="true"
      go_exists="true"
    fi
  ...
  if [ $is_install == "true" ]; then
    ...
    if [ $go_exists == "true" ]; then # remove old go if updating
      info "go: updating..."
      rm -rf dir_go
    else
      info "go: installing..."
...
check_git() {
  info "checking git"
  if ! command -v git &> /dev/null; then # git check
    install_pkg "git"
  else
...
check_zsh() {
  info "checking zsh"
  if [ "$SHELL" != "/usr/bin/zsh" ]; then # not zsh
    install_pkg "zsh"
...
check_tmux() {
  info "checking tmux"
  if ! command -v tmux &> /dev/null; then # tmux check
    install_pkg "tmux"
  else
...
check_vim() {
  info "checking vim"
  if ! command -v vim &> /dev/null; then # vim check
    install_pkg "vim"
    exit
  else
...
check_python_all() {
  ...
  if ! command -v python3 &> /dev/null; then #python3 check
    install_pkg "python3"
  else
  ...
  info "checking pip3"
  if ! command -v pip3 &> /dev/null; then #pip3 check
    install_pkg "pip3"
  else
